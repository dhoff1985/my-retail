...
class PriceFunctionalSpec extends BaseFunctionalSpec {
  ...
  def "put to price saves price"() {
    when:
    client.toBlocking().exchange(HttpRequest.PUT('/price/13860428', new Price('$3.95', CurrencyCode.USD)))
  ...
  def "put to price updates price"() {
    ...
    when:
    client.toBlocking().exchange(HttpRequest.PUT('/price/13860429', new Price('$3.95', CurrencyCode.USD)))
  ...
  @Unroll
  def "rejects incomplete/invalid requests"() {
    when:
    ...
    where:
    priceBody << [[:], [currency_code: 'USD'], [value: '$3.95'], [currency_code: 'EURO', value: '$3.95'], [currency_code: '', value: '$3.95'], [currency_code: 'USD', value: "abc"]]
  }
}
